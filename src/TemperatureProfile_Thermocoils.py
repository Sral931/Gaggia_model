...
solver.initialize(
    0.0,
    init_state,
    np.array([FULL_HEATER_POWER, 0.0])
)
...
for index in range(1,time_points):
    brew_delta: float64 = (500.0 + solver.state[model.STATE_INDEXES['water']] - temp_ambient)*flow[index]/100.0
    ...
    for i in range(40):
        solver.solve(
            inputs=np.array( [heater[index], flow[index]*0.8] ),
            timestep=time_step[index]/40.0
        )
